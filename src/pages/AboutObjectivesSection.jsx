...
export default function AboutObjectivesSection({ showBanner, shouldAnimateBanner, onNavigate }) {
  ...
  const heading = about?.objectives?.heading || 'Our Objectives';
  const body = about?.objectives?.body;
        ...
        <div className="flex items-center justify-center h-full gap-8 px-8">
          {tabs.map(t => (
            <button
              ...
              className={`text-sm transition-all ${t.nav === 'about-objectives' ? 'font-semibold text-white underline hover:opacity-70' : 'font-medium text-white/70 hover:text-white hover:underline'}`}
            >
