...
export default function AboutTeamSection({ showBanner, shouldAnimateBanner, onNavigate }) {
  ...
  const heading = about?.team?.heading || 'Our Team';
  const body = about?.team?.body;
        ...
        <div className="flex items-center justify-center h-full gap-8 px-8">
          {tabs.map(t => (
            <button
              ...
              className={`text-sm transition-all ${t.nav === 'about-team' ? 'font-semibold text-white underline hover:opacity-70' : 'font-medium text-white/70 hover:text-white hover:underline'}`}
            >
              ...
              : (
                <>
                  <p className="text-lg leading-relaxed mb-4 text-white/70">
                    [Detailed content about BrainMoove's team will go here...]
                  </p>
