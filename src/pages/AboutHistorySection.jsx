...
export default function AboutHistorySection({ showBanner, shouldAnimateBanner, onNavigate }) {
  ...
  const heading = about?.history?.heading || 'History of BrainMoove';
  const body = about?.history?.body;
        ...
        <div className="flex items-center justify-center h-full gap-8 px-8">
          {tabs.map(t => (
            <button
              ...
              className={`text-sm transition-all ${t.nav === 'about-history' ? 'font-semibold text-white underline hover:opacity-70' : 'font-medium text-white/70 hover:text-white hover:underline'}`}
            >
